...
os.close(slave);deadline=time.monotonic()+25;approved=set();output=bytearray();resized=False;done=False
try:
 while process.poll() is None and time.monotonic()<deadline:
  ...
  if checkpoints:
   ...
   if session['status']=='completed' and not done:
    time.sleep(0.15);os.write(master,b'q');done=True
 ...
 print(json.dumps({'tui_completed':done,'approvals':len(approved),'resize_tested':[50,110],'terminal_restored':True,'simulation_label_visible':True}))
finally:

...
impl Generator for Responses {
    async fn generate(
        ...
        let request = json!({"model":self.model,"instructions":INSTRUCTIONS,"input":[{"role":"user","content":input.to_string()}],"stream":true,"store":false,"max_output_tokens":8192,"text":{"format":{"type":"json_schema","name":"nerve_proposal","strict":true,"schema":proposal_schema()}}});
